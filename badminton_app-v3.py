...
# ---------------------------------------------------------
st.markdown("<h1 style='text-align: center; color: #1E3A8A;'>🏸 SUNDAY SMASH CLUB</h1>", unsafe_allow_html=True)
st.write("---")
                    ...
                    with st.expander("📝 Nhập Chi Phí Sân & Cầu + Chia Tiền Chi Tiết (Chỉ Host)", expanded=False):
                        ...
                        with st.form(f"calc_fee_form_{sess_id}"):
                            ...
                            for pl in current_players:
                                ...
                                with col_p1:
                                    coeff = st.number_input("Hệ số chia tiền", min_value=0.0, max_value=2.0, value=float(pl['coefficient']), step=0.05, key=f"coeff_{p_id}")
